...
namespace nilnul.bit.abgebra_.cumula.str
{
	[TestClass]
	public class UnitTest1
	{
		...
		{
			var op8outcomeEs = nilnul.bit.op_.binary.Sortie._Arr;

			var els = nilnul.bit.Sortie.ARR;

			var cos = nilnul.bit.co._SortieX.Array;
			...
			foreach (var op8outcome in op8outcomeEs)
			{
				...
				foreach (var item in els)
				{
					...
					var op1 =  nilnul.bit.Bi.Of( op8outcome);
					var cumula = new nilnul.bit.op_.binary.Cumulator(item, op1);
					...
					var inversesLeft = new Dictionary<bool, IEnumerable<bool>>();

					var inversesRight = new Dictionary<bool, IEnumerable<bool>>();

					var inverses = new Dictionary<bool, IEnumerable<bool>>();

					foreach (var item11 in bit.Sortie.ARR)
					{
						var valLeft = new List<bool>();
						var valRight = new List<bool>();
						var val = new List<bool>();

						foreach (var item222 in bit.Sortie.ARR)
						{
						...
						inversesLeft.Add( (item11), valLeft);
						inversesRight.Add(( item11), valRight);
						inverses.Add( (
							item11), val
						);
					}
					...
					r.ItemArray = new object[] {
						...
						nilnul.bit.op_.binary.be_._CommutableX.Be(op8outcome).ToNum().ToString()
						,
						nilnul.bit.op_.binary.be_._AssociativeX.Be(op8outcome).ToNum().ToString()
						,
						item.ToNum().ToString()
						,
						...
						nilnul.bit.bi.cumula.be_.Generative.Singleton.be(cumula).ToNum()

						,
						nilnul.bit.bi.cumula.be_.gen_.Cyclic.Singleton.be(cumula).ToNum()
			...
			static string dict2txt(Dictionary<bool, IEnumerable<bool>> inversesLeft)
			{
				return string.Join(";",

									inversesLeft.Select(kv => $"{kv.Key.ToNum()}:{nilnul.bits._TxtenX.NoSep(kv.Value)}")
